...
company_change <- Full %>%
  mutate(
    ...
    company_level_2018 <- company_change %>%
      filter(Inception_Year <= 2018) %>% 
      mutate(
        Count_2018 =
          case_when(
            is.na(Obsolete_Year) ~ "Exists_2018",
            Obsolete_Year <= 2018 ~ "Obsolete_2018",
            Obsolete_Year > 2018 ~ "Exists_2018",
            TRUE ~ "Other"
      ))
    ...
    company_level_change <- company_level_change %>%
      ...
      summarise(Exists_2022 = sum(Exists_2022, na.rm = T),
                ...
                Change = sum(Exists_2022,-Exists_2018, na.rm = T))
    ...
    company_funds <- company_change %>%
      ...
      group_by(Branding.Name.Mod,
               Name.of..Fund.s..Oldest.Share.Class,
               Newest_ObDate,
               Oldest_InDate) %>%
      summarise() %>%
      mutate(
